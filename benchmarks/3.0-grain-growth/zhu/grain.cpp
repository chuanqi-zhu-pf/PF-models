...
double gaij[N][N], aij[N][N], wij[N][N], mij[N][N], fij[N][N];
...
double F0;					   //粒界移動の駆動力
double temp;				   //温度
double sum1, sum2, sum3, sumi; //各種の和の作業変数
...
start:;
	...
	if (istep % pstep == 0)
	{
		std::cout << "interface energy is " << sumi / vm0 * RR * temp * dx << std::endl;
		datasave1d(istep);
	}
				...
				for (n1 = 1; n1 <= phiNum[i][j][k]; n1++)
				{
					...
					for (n2 = 1; n2 <= phiNum[i][j][k]; n2++)
					{
						...
						for (n3 = 1; n3 <= phiNum[i][j][k]; n3++)
						{
							...
							termiikk = gaij[ii][kk] * ((phidxx + phidyy + phidzz) + PI * PI / delta / delta * (phi[kk][i][j][k] - 0.5));
							termjjkk = gaij[jj][kk] * ((phidxx + phidyy + phidzz) + PI * PI / delta / delta * (phi[kk][i][j][k] - 0.5));

							sum1 += termiikk - termjjkk;
						}
						pddtt += -2.0 * mobi / (double)phiNum[i][j][k] * (sum1 - PI * fij[ii][jj] / delta * sqrt(phi[ii][i][j][k] * phi[jj][i][j][k]));
	...
	for (i = 0; i <= ndmx; i++)
	{
		for (j = 0; j <= ndmy; j++)
		{
			for (k = 0; k <= ndmz; k++)
			{
				phidx = (phi[1][ip][j][k] - phi[1][im][j][k]) / 2.0;
				sumi += (phidx * phidx * delta * delta / PI / PI + phi[1][i][j][k] * (1.0 - phi[1][i][j][k])) * 4.0 * gamma0 / delta;
				// phidx = (phi[0][ip][j][k] - phi[0][im][j][k]) / 2.0;
				// sumi += (phidx * phidx * delta * delta / PI / PI + phi[0][i][j][k] * (1.0 - phi[0][i][j][k])) * 4.0 * gamma0 / delta;
			}
		}
	}
