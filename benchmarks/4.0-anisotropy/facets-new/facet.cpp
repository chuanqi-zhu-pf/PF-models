...
double phidxxk, phidyyk, phidzzk;
...
double th, vp, eta;
double epsilon0;
double gaiikk, gaiikk0, gajjkk, gajjkk0, gadx, gady, gadz;
double termx, termy, termz;
double termiikk, termjjkk, wwiikk, wwjjkk;

double phidxk, phidyk, phidzk, phiabs2k, phiabsk;
double phidxyk, phidyzk, phidxzk;
double phidyxk, phidzyk, phidzxk;
...
double gadphix, gadphiy, gadphiz;
double gadphixdx, gadphiydy, gadphizdz;
				...
				for (n1 = 1; n1 <= phiNum[i][j][k]; n1++)
				{
					...
					for (n2 = 1; n2 <= phiNum[i][j][k]; n2++)
					{
						...
						for (n3 = 1; n3 <= phiNum[i][j][k]; n3++)
						{
							...
							phidxk = (phi[kk][ip][j][k] - phi[kk][im][j][k]) / 2.0;
							phidyk = (phi[kk][i][jp][k] - phi[kk][i][jm][k]) / 2.0;
							phidzk = (phi[kk][i][j][kp] - phi[kk][i][j][km]) / 2.0;

							phidxxk = (phi[kk][ip][j][k] + phi[kk][im][j][k] - 2.0 * phi[kk][i][j][k]); //フェーズフィールドの空間２階微分
							phidyyk = (phi[kk][i][jp][k] + phi[kk][i][jm][k] - 2.0 * phi[kk][i][j][k]);
							phidzzk = (phi[kk][i][j][kp] + phi[kk][i][j][km] - 2.0 * phi[kk][i][j][k]);

							//------------------------------ Implementation of gradeint term with cubic anisotropy ---------------------

							phidx = (phi[ii][ip][j][k] - phi[ii][im][j][k]) / 2.0;
							phidy = (phi[ii][i][jp][k] - phi[ii][i][jm][k]) / 2.0;
							phidz = (phi[ii][i][j][kp] - phi[ii][i][j][km]) / 2.0;

							phidxx = (phi[ii][ip][j][k] + phi[ii][im][j][k] - 2.0 * phi[ii][i][j][k]);
							phidyy = (phi[ii][i][jp][k] + phi[ii][i][jm][k] - 2.0 * phi[ii][i][j][k]);
							phidzz = (phi[ii][i][j][kp] + phi[ii][i][j][km] - 2.0 * phi[ii][i][j][k]);

							phidxy = (phi[ii][ip][jp][k] + phi[ii][im][jm][k] - phi[ii][im][jp][k] - phi[ii][ip][jm][k]) / 4.0;
							phidxz = (phi[ii][ip][j][kp] + phi[ii][im][j][km] - phi[ii][im][j][kp] - phi[ii][ip][j][km]) / 4.0;
							phidyz = (phi[ii][i][jp][kp] + phi[ii][i][jm][km] - phi[ii][i][jm][kp] - phi[ii][i][jp][km]) / 4.0;
							...
							if (anij[ii][kk] == 2.0 && phiabs2 != 0.0)
							{
								...
								gaiikk = gaiikk0 * (1.0 + del * (CC + tan(al0) * SS)) * coef;
								...
								gadx = gaiikk0 * del * (1.0 / (2.0 * uu * CC) - tan(al0) / (2.0 * uu * SS)) * (2.0 * dPdx + dQdx) * coef;
								gady = gaiikk0 * del * (1.0 / (2.0 * uu * CC) - tan(al0) / (2.0 * uu * SS)) * (2.0 * dPdy + dQdy) * coef;
								gadz = gaiikk0 * del * (1.0 / (2.0 * uu * CC) - tan(al0) / (2.0 * uu * SS)) * (2.0 * dPdz + dQdz) * coef;

								gadphix = gaiikk0 * del * (1.0 / (2.0 * uu * CC) - tan(al0) / (2.0 * uu * SS)) * (2.0 * dPdphix + dQdphix) * coef;
								gadphiy = gaiikk0 * del * (1.0 / (2.0 * uu * CC) - tan(al0) / (2.0 * uu * SS)) * (2.0 * dPdphiy + dQdphiy) * coef;
								gadphiz = gaiikk0 * del * (1.0 / (2.0 * uu * CC) - tan(al0) / (2.0 * uu * SS)) * (2.0 * dPdphiz + dQdphiz) * coef;

								gadphixdx = 1.0 / 2.0 / uu * gaiikk0 * del * ((-1.0 / 2.0 / uu / pow(CC, 3.0) - tan(al0) / (2.0 * uu * pow(SS, 3.0))) * (2.0 * dPdx + dQdx) * (2.0 * dPdphix + dQdphix) + (2.0 * dPdphixdx + dQdphixdx) * (1.0 / CC - tan(al0) / SS)) * coef;
								gadphiydy = 1.0 / 2.0 / uu * gaiikk0 * del * ((-1.0 / 2.0 / uu / pow(CC, 3.0) - tan(al0) / (2.0 * uu * pow(SS, 3.0))) * (2.0 * dPdy + dQdy) * (2.0 * dPdphiy + dQdphiy) + (2.0 * dPdphiydy + dQdphiydy) * (1.0 / CC - tan(al0) / SS)) * coef;
								gadphizdz = 1.0 / 2.0 / uu * gaiikk0 * del * ((-1.0 / 2.0 / uu / pow(CC, 3.0) - tan(al0) / (2.0 * uu * pow(SS, 3.0))) * (2.0 * dPdz + dQdz) * (2.0 * dPdphiz + dQdphiz) + (2.0 * dPdphizdz + dQdphizdz) * (1.0 / CC - tan(al0) / SS)) * coef;

								termx = gaiikk * (phidxxk - phidxx) + gadx * (phidxk - phidx) + gadphixdx * (phidx * phidxk) + gadphix * (phidxx * phidxk + phidx * phidxxk);
								termy = gaiikk * (phidyyk - phidyy) + gady * (phidyk - phidy) + gadphiydy * (phidy * phidyk) + gadphiy * (phidyy * phidyk + phidy * phidyyk);
								termz = gaiikk * (phidzzk - phidzz) + gadz * (phidzk - phidz) + gadphizdz * (phidz * phidzk) + gadphiz * (phidzz * phidzk + phidz * phidzzk);

								termiikk = termx + termy + termz;
							}
							else
							{
								termiikk = gaij[ii][kk] * (phidxxk + phidyyk + phidzzk + PI * PI / delta / delta * phi[kk][i][j][k]);
							}

							phidx = (phi[jj][ip][j][k] - phi[jj][im][j][k]) / 2.0;
							phidy = (phi[jj][i][jp][k] - phi[jj][i][jm][k]) / 2.0;
							phidz = (phi[jj][i][j][kp] - phi[jj][i][j][km]) / 2.0;

							phidxx = (phi[jj][ip][j][k] + phi[jj][im][j][k] - 2.0 * phi[jj][i][j][k]);
							phidyy = (phi[jj][i][jp][k] + phi[jj][i][jm][k] - 2.0 * phi[jj][i][j][k]);
							phidzz = (phi[jj][i][j][kp] + phi[ii][i][j][km] - 2.0 * phi[jj][i][j][k]);

							phidxy = (phi[jj][ip][jp][k] + phi[jj][im][jm][k] - phi[jj][im][jp][k] - phi[jj][ip][jm][k]) / 4.0;
							phidxz = (phi[jj][ip][j][kp] + phi[jj][im][j][km] - phi[jj][im][j][kp] - phi[jj][ip][j][km]) / 4.0;
							phidyz = (phi[jj][i][jp][kp] + phi[jj][i][jm][km] - phi[jj][i][jm][kp] - phi[jj][i][jp][km]) / 4.0;
							...
							if (anij[jj][kk] == 2.0 && phiabs2 != 0.0)
							{
								...
								gajjkk = gajjkk0 * (1.0 + del * (CC + tan(al0) * SS)) * coef;
								...
								gadx = gajjkk0 * del * (1.0 / (2.0 * uu * CC) - tan(al0) / (2.0 * uu * SS)) * (2.0 * dPdx + dQdx) * coef;
								gady = gajjkk0 * del * (1.0 / (2.0 * uu * CC) - tan(al0) / (2.0 * uu * SS)) * (2.0 * dPdy + dQdy) * coef;
								gadz = gajjkk0 * del * (1.0 / (2.0 * uu * CC) - tan(al0) / (2.0 * uu * SS)) * (2.0 * dPdz + dQdz) * coef;

								gadphix = gajjkk0 * del * (1.0 / (2.0 * uu * CC) - tan(al0) / (2.0 * uu * SS)) * (2.0 * dPdphix + dQdphix) * coef;
								gadphiy = gajjkk0 * del * (1.0 / (2.0 * uu * CC) - tan(al0) / (2.0 * uu * SS)) * (2.0 * dPdphiy + dQdphiy) * coef;
								gadphiz = gajjkk0 * del * (1.0 / (2.0 * uu * CC) - tan(al0) / (2.0 * uu * SS)) * (2.0 * dPdphiz + dQdphiz) * coef;

								gadphixdx = 1.0 / 2.0 / uu * gajjkk0 * del * ((-1.0 / 2.0 / uu / pow(CC, 3.0) - tan(al0) / (2.0 * uu * pow(SS, 3.0))) * (2.0 * dPdx + dQdx) * (2.0 * dPdphix + dQdphix) + (2.0 * dPdphixdx + dQdphixdx) * (1.0 / CC - tan(al0) / SS)) * coef;
								gadphiydy = 1.0 / 2.0 / uu * gajjkk0 * del * ((-1.0 / 2.0 / uu / pow(CC, 3.0) - tan(al0) / (2.0 * uu * pow(SS, 3.0))) * (2.0 * dPdy + dQdy) * (2.0 * dPdphiy + dQdphiy) + (2.0 * dPdphiydy + dQdphiydy) * (1.0 / CC - tan(al0) / SS)) * coef;
								gadphizdz = 1.0 / 2.0 / uu * gajjkk0 * del * ((-1.0 / 2.0 / uu / pow(CC, 3.0) - tan(al0) / (2.0 * uu * pow(SS, 3.0))) * (2.0 * dPdz + dQdz) * (2.0 * dPdphiz + dQdphiz) + (2.0 * dPdphizdz + dQdphizdz) * (1.0 / CC - tan(al0) / SS)) * coef;

								termx = gajjkk * (phidxxk - phidxx) + gadx * (phidxk - phidx) + gadphixdx * (phidx * phidxk) + gadphix * (phidxx * phidxk + phidx * phidxxk);
								termy = gajjkk * (phidyyk - phidyy) + gady * (phidyk - phidy) + gadphiydy * (phidy * phidyk) + gadphiy * (phidyy * phidyk + phidy * phidyyk);
								termz = gajjkk * (phidzzk - phidzz) + gadz * (phidzk - phidz) + gadphizdz * (phidz * phidzk) + gadphiz * (phidzz * phidzk + phidz * phidzzk);

								termjjkk = termx + termy + termz;
							}
							else
							{
								termjjkk = gaij[jj][kk] * (phidxxk + phidyyk + phidzzk + PI * PI / delta / delta * phi[kk][i][j][k]);
							}

							//-----------------------------------------------------------------------------------------------------------
							sum1 += termjjkk - termiikk;
						}
						pddtt += muij[ii][jj] / (double)phiNum[i][j][k] * (sum1 + 2.0 * PI / delta * fij[ii][jj] * sqrt(phi[ii][i][j][k] * phi[jj][i][j][k]));
