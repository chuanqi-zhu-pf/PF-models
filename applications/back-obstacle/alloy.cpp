			...
			for (j = 0; j <= ndmy; j++)
			{
				for (k = 0; k <= ndmz; k++)
				{
					...
					if (phi[0][i][j][k] > 0.0 && phi2[0][i][j][k] < 1.0)
					{
						...
						conp[1][i][j][k] = (conp[1][i][j][k] * phi[1][i][j][k] + (phi2[1][i][j][k] - phi[1][i][j][k]) * (kvi * coni[i][j][k])) / phi2[1][i][j][k];
						...
						// {
							...
							do
							{
								...
								if (phi2[0][ixdi][jydi][k] == 1.0)
								{
									...
									flxci = (phi2[1][i][j][k] - phi[1][i][j][k]) * coni[i][j][k] * (1.0 - kvi);
									con[i][j][k] -= flxci;
				...
				for (k = 0; k <= ndmz; k++)
				{
					...
					if (phi[0][i][j][k] < 1.0 && phi[0][i][j][k] > 0.0)
					{
						...
						do
						{
							...
							if (phi[0][ixdip][jydip][k] == 0.0)
							{
								coni[i][j][k] = conp[0][ixdi][jydi][k];
								break;
							}

						} while (phi[0][ixdip][jydip][k] > 0.0);
					}
				}
			}
		}
...
void datasave2d(int step)
{
	...
	for (ix = 0; ix <= ndmx; ix++)
	{
		for (iy = 0; iy <= ndmy; iy++)
		{
			fprintf(streamp, "%lf   ", coni[ix][iy][0]);
