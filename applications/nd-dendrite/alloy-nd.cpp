...
int main()
{
	dT = 1.10; // undercooling below melting point
	cl = 0.01; // initial liquid concentraion

	kap1 = 0.100;			 // partition coefficient
	dT0 = (1.0 - kap1) * dT; // solidification range ~1.0K

	ml1 = -60.0;
	c10e = -dT / ml1;
	c1e = c10e * kap1;

	gt0 = 1.0;			// Gibbs-Thomson coefficient
	dd0 = gt0 / dT0;	// Capilarity length 1.0
	delta = 50.0 * dd0; // W0/d0 
	dx = delta / 5.0;

	ll0 = 250.0 * dd0; // diffusion length 
	dTi = dT0 / 10.0;  // interface undercooling
	mobi = 3.0;
	Dl = ll0 * mobi * dTi;
	Ds = Dl * 1.0e-3;
	dtime = 0.2 * dx * dx / Dl;
		...
		if (th_id == 0)
		{
			...
			}else
			{
				if((intpos-intposp)>1)
				{
					...
					fprintf(stream, "%lf   ", istep * dtime);
					fprintf(stream, "%lf   ", (intpos - intposp) * dx);
					fprintf(stream, "%lf   ", (istep - istepp) * dtime);
					fprintf(stream, "\n");
					fclose(stream);
					intposp = intpos;
					istepp = istep;
