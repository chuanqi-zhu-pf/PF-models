...
double temp0, gamma0, delta, mobi;
double M0, W0, A0;

int ni, nj, ix, iy, iz;
double sumc, c0, dc0;
double cl, Tm, S0, dT0, dTi, dd0, ll0, ml, kap;
double c1e, c0e;
double Dl, Ds;
double astre, astrem;
...
int main()
{
	...
	for (ix = 0; ix <= ndmx; ix++)
	{
		for (iy = 0; iy <= ndmy; iy++)
		{
			...
			{
				// if (i < NDX / 8)
				// if ((ix-NDX/2) * (ix-NDX/2) + (iy-NDY/2) * (iy-NDY/2) < NDX / 8 * NDX / 8)
				if (ix + iy < NDX / 8)
				{
					phi[1][ix][iy][iz] = 1.0;
					conp[1][ix][iy][iz] = c1e;
...
void datasave1d(int step)
{
	...
	for (ix = 0; ix <= ndmx; ix++)
	{
		fprintf(streamc, "%lf   ", coni[ix][0][0]);
		fprintf(streamc, "\n");
	}
	fclose(streamc);
}
...
void datasave2d(int step)
{
	...
	for (ix = 0; ix <= ndmx; ix++)
	{
		for (iy = 0; iy <= ndmy; iy++)
		{
			fprintf(streamc, "%lf   ", coni[ix][iy][0]);
			fprintf(streamc, "\n");
		}
	}
	...
	for (ix = 0; ix <= ndmx; ix++)
	{
		for (iy = 0; iy <= ndmy; iy++)
		{
			fprintf(streamcp, "%lf   ", conp[0][ix][iy][0]);
			fprintf(streamcp, "\n");
		}
	}
