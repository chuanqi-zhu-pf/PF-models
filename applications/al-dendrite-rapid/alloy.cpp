	...
	for (ix = 0; ix <= ndmx; ix++)
	{
		for (iy = 0; iy <= ndmy; iy++)
		{
			for (iz = 0; iz <= ndmz; iz++)
			{
				...
				// if (ix < NDX / 32)
				if ((ix) * (ix) + (iy - NDY / 2) * (iy - NDY / 2) < NDX / 32 * NDX / 32)
				// if (ix < NDX * 1.2 / 2 && iy < NDY / 12)
				{
					phi[1][ix][iy][iz] = 1.0;
				...
				// else if (ix < NDX * 1.2 / 2 && iy >= NDY * 5 / 12 && iy < NDY * 7 / 12)
				// {
				// 	phi[1][ix][iy][iz] = 1.0;
				// 	conp[1][ix][iy][iz] = calCse(1, temp[ix][iy][iz]);
				// 	phi[0][ix][iy][iz] = 0.0;
				// 	conp[0][ix][iy][iz] = calCle(1, temp[ix][iy][iz]);
				// }
				// else if (ix < NDX * 1.2 / 2 && iy >= NDY * 11 / 12)
				// {
				// 	phi[1][ix][iy][iz] = 1.0;
				// 	conp[1][ix][iy][iz] = calCse(1, temp[ix][iy][iz]);
				// 	phi[0][ix][iy][iz] = 0.0;
					...
					for (j = 0; j <= ndmy; j++)
					{
						for (k = 0; k <= ndmz; k++)
						{
							...
							else
							{
								...
								phi[1][i][j][k] = 0.0;
								conp[0][i][j][k] = cl;
								conp[1][i][j][k] = calCse(1, temp[ix][iy][iz]);
			...
			for (j = 0; j <= ndmy; j++)
			{
				for (k = 0; k <= ndmz; k++)
				{
					...
					if (phi[0][i][j][k] < 1.0 && phi[0][i][j][k] > 0.0)
					{
						...
						if (phi[0][i][j][k] >= 0.5 && conp[0][i][j][k] < calCle(1, temp[i][j][k]))
					...
					// 		} while (phi[0][ixdi][jydi][kzdi] >= 0.5 && abs(di) <= (int(delta / dx / 2)));
					// 		if (abs(di) > (int(delta / dx / 2)))
					// 		{
					// 			coni[i][j][k] = calCle(1, temp[i][j][k]);
					// 		}
					// 	}
					// }
				}
...
void datasave1d(int step)
{
	...
	char buffercs[30];
	sprintf(buffercs, "data/temp/1d%d.csv", step);
	streamcs = fopen(buffercs, "a"); // 書き込む先のファイルを追記方式でオープン

	for (ix = 0; ix <= ndmx; ix++)
	{
		fprintf(streamcs, "%lf   ", temp[ix][0][0]);
		fprintf(streamcs, "\n");
	}
