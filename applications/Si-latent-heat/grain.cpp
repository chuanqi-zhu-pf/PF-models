...
double Tm, Dt, lh0, sh0, tddtt, Tr, Tg;
int intpos, intdis;
		...
		for (j = 0; j <= ndmy; j++)
		{
			for (k = 0; k <= ndmz; k++)
			{
				...
				for (n1 = 1; n1 <= phiNum[i][j][k]; n1++)
				{
					...
					tddtt = Dt * (temp[ip][j][k] + temp[im][j][k] + temp[i][jp][k] + temp[i][jm][k] - 4.0 * temp[i][j][k]) / dx / dx + lh0 * pddtt / sh0;
