...
int ni, nj, ix, iy, iz;
double sumc, c0, dc0;
double cl, dT, dT0, dTi, dd0, ll0, ml1, kap1;
double c1e, c10e;
double Dl, Ds;
double astre, astrem;
...
int main()
{
	kap1 = 0.100; // partition coefficient
	dT0 = 10.0;	  // solidification range ~10.0K

	cl = 0.01;	  // initial liquid concentraion
	c10e = 0.018; // equilibrium liquid concentration
	c1e = c10e * kap1;

	gt0 = 2.4e-7;		// Gibbs-Thomson coefficient
	dd0 = gt0 / dT0;	// Capilarity length
	delta = 50.0 * dd0; // W0/d0
	dx = delta / 5.0;
	...
	for (ix = 0; ix <= ndmx; ix++)
	{
		for (iy = 0; iy <= ndmy; iy++)
		{
			...
			{
				// if (i < NDX / 8)
				// if ((ix - NDX / 2) * (ix - NDX / 2) + (iy - NDX / 2) * (iy - NDX / 2) < NDX / 16 * NDX / 16)
				if (ix + iy < NDX / 32)
				{
					phi[1][ix][iy][iz] = 1.0;
					conp[1][ix][iy][iz] = c1e;
...
#pragma omp parallel num_threads(NTH)
	{
		...
		double conp0ip, conp0im, conp0jp, conp0jm, conp0kp, conp0km;
		double phi0ip, phi0im, phi0jp, phi0jm;
		double pddc, dpdc;
				...
				for (k = 0; k <= ndmz; k++)
				{
					...
					for (n1 = 1; n1 <= phiNum[i][j][k]; n1++)
					{
						...
						for (n2 = 1; n2 <= phiNum[i][j][k]; n2++)
						{
							...
							if (ii == 1 && jj == 0)
							{
								dT = (1.0 - coni[i][j][k] / c10e) / (1.0 - kap1) * dT0;
							}
							else if (ii == 0 && jj == 1)
							{
								dT = -(1.0 - coni[i][j][k] / c10e) / (1.0 - kap1) * dT0;
		...
		for (i = start; i <= end; i++)
		{
			for (j = 0; j <= ndmy; j++)
			{
				for (k = 0; k <= ndmz; k++)
				{
					...
					fes1 = Ds * (conp[1][ip][j][k] * phi[1][i][j][k] - conp[1][i][j][k] * phi[1][i][j][k]) / dx;
					fws1 = Ds * (conp[1][i][j][k] * phi[1][i][j][k] - conp[1][im][j][k] * phi[1][i][j][k]) / dx;
					if (phi[1][ip][j][k] == 0.0)
					{
						fes1 = 0.0;
					}
					...
					fns1 = Ds * (conp[1][i][jp][k] * phi[1][i][j][k] - conp[1][i][j][k] * phi[1][i][j][k]) / dx;
					fss1 = Ds * (conp[1][i][j][k] * phi[1][i][j][k] - conp[1][i][jm][k] * phi[1][i][j][k]) / dx;
					if (phi[1][i][jp][k] == 0.0)
					{
						fns1 = 0.0;
					}
					...
					fos1 = Ds * (conp[1][i][j][kp] * phi[1][i][j][k] - conp[1][i][j][k] * phi[1][i][j][k]) / dx;
					fis1 = Ds * (conp[1][i][j][k] * phi[1][i][j][k] - conp[1][i][j][km] * phi[1][i][j][k]) / dx;
					if (phi[1][i][j][kp] == 0.0)
					{
						fos1 = 0.0;
					}
					...
					if ((phi[0][i][j][k] >= 0.5 && phi[0][ip][j][k] < 0.5))
					{
						conp0ip = conp[0][i][j][k];
					}
					if ((phi[0][i][j][k] >= 0.5 && phi[0][im][j][k] < 0.5))
					{
						conp0im = conp[0][i][j][k];
					}
					if ((phi[0][i][j][k] >= 0.5 && phi[0][i][jp][k] < 0.5))
					{
						conp0jp = conp[0][i][j][k];
					}
					if ((phi[0][i][j][k] >= 0.5 && phi[0][i][jm][k] < 0.5))
					{
						conp0jm = conp[0][i][j][k];
					}
					if ((phi[0][i][j][k] >= 0.5 && phi[0][i][j][kp] < 0.5))
					{
						conp0kp = conp[0][i][j][k];
					}
					if ((phi[0][i][j][k] >= 0.5 && phi[0][i][j][km] < 0.5))
					{
						conp0km = conp[0][i][j][k];
					}
					...
					pddc = Dl * phi[0][i][j][k] * (conp0ip + conp0im + conp0jp + conp0jm + conp0kp + conp0km - 6.0 * conp[0][i][j][k]) / dx / dx;
					...
					dpdc = Dl * ((phi[0][ip][j][k] - phi[0][im][j][k]) * (conp0ip - conp0im) + (phi[0][i][jp][k] - phi[0][i][jm][k]) * (conp0jp - conp0jm) + (phi[0][i][j][kp] - phi[0][i][j][km]) * (conp0kp - conp0km)) / 4.0 / dx / dx;
...
void datasave2d(int step)
{
	...
	for (ix = 0; ix <= ndmx; ix++)
	{
		for (iy = 0; iy <= ndmy; iy++)
		{
			fprintf(streamc, "%lf   ", conp[0][ix][iy][0]);
			fprintf(streamc, "\n");
		}
	}
	...
	for (ix = 0; ix <= ndmx; ix++)
	{
		for (iy = 0; iy <= ndmy; iy++)
		{
			fprintf(streamcp, "%lf   ", con[ix][iy][0]);
			fprintf(streamcp, "\n");
		}
	}
