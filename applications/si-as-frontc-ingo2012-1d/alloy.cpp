...
double con[NDX][NDY][NDZ], coni[NDX][NDY][NDZ], veli[NDX][NDY][NDZ], consi[NDX][NDY][NDZ];
double sumc, c0, dc0;
double cl, Tm, ml, kap;
double cse, cs0e;
double dT, S0, Dl, Ds;
...
double calcCse(double veli, double conpi);
...
int main()
{
	...
	if (0.2 / mobi / gamma0 < dtime)
	{
		dtime = 0.2 / mobi / gamma0;
	}
...
start:;
	...
	{
		datasave1d(istep);
		std::cout << dtime * istep << " (s) has passed" << std::endl;
	}
		...
		for (j = 0; j <= ndmy; j++)
		{
			for (k = 0; k <= ndmz; k++)
			{
				...
				if (phi[0][i][j][k] > 0.0 && phi2[0][i][j][k] < 1.0)
				{
					...
					if (phi2[0][i][j][k] != 0.0)
					{
						veli[i][j][k] = -(phi2[0][i][j][k] - phi[0][i][j][k]) / sqrt(phidxi * phidxi + phidyi * phidyi) / dtime;
					}
					...
					do
					{
						...
						{
							flxci = -(phi[0][i][j][k] - phi2[0][i][j][k]) * (calcCse(veli[i][j][k], coni[i][j][k]) - conp[0][i][j][k]);
							con[i][j][k] -= flxci;
	...
	for (i = 0; i <= ndmx; i++)
	{
		for (j = 0; j <= ndmy; j++)
		{
			for (k = 0; k <= ndmz; k++)
			{
				...
				fes = Ds * ((phi[1][i][j][k] + phi[2][i][j][k]) * (conp[1][ip][j][k] - conp[1][i][j][k]));
				fws = Ds * ((phi[1][i][j][k] + phi[2][i][j][k]) * (conp[1][i][j][k] - conp[1][im][j][k]));
				...
				fns = Ds * ((phi[1][i][j][k] + phi[2][i][j][k]) * (conp[1][i][jp][k] - conp[1][i][j][k]));
				fss = Ds * ((phi[1][i][j][k] + phi[2][i][j][k]) * (conp[1][i][j][k] - conp[1][i][jm][k]));
				...
				fos = Ds * ((phi[1][i][j][k] + phi[2][i][j][k]) * (conp[1][i][j][kp] - conp[1][i][j][k]));
				fis = Ds * ((phi[1][i][j][k] + phi[2][i][j][k]) * (conp[1][i][j][k] - conp[1][i][j][km]));
				...
				fel = Dl * phi[0][i][j][k] * (conp[0][ip][j][k] - conp[0][i][j][k]);
				fwl = Dl * phi[0][i][j][k] * (conp[0][i][j][k] - conp[0][im][j][k]);
				if (phi[0][ip][j][k] == 0.0)
				{
					fel = 0.0;
				}
				...
				fnl = Dl * phi[0][i][j][k] * (conp[0][i][jp][k] - conp[0][i][j][k]);
				fsl = Dl * phi[0][i][j][k] * (conp[0][i][j][k] - conp[0][i][jm][k]);
				if (phi[0][i][jp][k] == 0.0)
				{
					fnl = 0.0;
				}
				...
				fol = Dl * phi[0][i][j][k] * (conp[0][i][j][kp] - conp[0][i][j][k]);
				fil = Dl * phi[0][i][j][k] * (conp[0][i][j][k] - conp[0][i][j][km]);
				if (phi[0][i][j][kp] == 0.0)
				{
					fol = 0.0;
				}
				...
				cddttl = (fel - fwl + fnl - fsl + fol - fil);
				cddtts1 = (fes - fws + fns - fss + fos - fis);
