...
double mophi, pho;
int mstep;
int istepp, intpos, intposp, intdis, passdis;
int allL;
				...
				for (k = 0; k <= ndmz; k++)
				{
					...
					for (n1 = 1; n1 <= phiNum[i][j][k]; n1++)
					{
						...
						for (n2 = 1; n2 <= phiNum[i][j][k]; n2++)
						{
							...
							if (ii == 1 && jj == 0)
							{
								dT = Tm + coni[i][j][k] * ml1 - temp[i][j][k];
							}
							else if (ii == 0 && jj == 1)
							{
								dT = -(Tm + coni[i][j][k] * ml1 - temp[i][j][k]);
			...
			for (j = 0; j <= ndmy; j++)
			{
				for (k = 0; k <= ndmz; k++)
				{
					if (phi[0][i][j][k] == 0.0)
					{
						...
						for (ii = 1; ii <= nm; ii++)
						{
							conp[0][i][j][k] += calC0e(ii, temp[i][j][k]) * phi[ii][i][j][k];
						}
					}
...
void datasave1d(int step)
{
	...
	for (ix = 0; ix <= ndmx; ix++)
	{
		fprintf(stream, "%lf   ", phi[0][ix][0][0]);
		fprintf(stream, "\n");
	}
	...
	for (ix = 0; ix <= ndmx; ix++)
	{
		fprintf(streamcs, "%lf   ", vii[ix][0][0]);
		fprintf(streamcs, "\n");
	}
