...
int main()
{
	...
	for (ix = 0; ix <= ndmx; ix++)
	{
		for (iy = 0; iy <= ndmy; iy++)
		{
			...
			{
				mv[ix][iy][iz] = ml1;
				temp[ix][iy][iz] = temp0 - NDX / 32 * dx * Tg + Tg * ix * dx;
...
start:;
	...
	else
	{
		if ((intpos - intposp) > 1)
		{
			...
			char buffer[30];
			sprintf(buffer, "data/tip_pos.csv");
			...
			for (k = 0; k <= ndmz; k++)
			{
				...
				for (n1 = 1; n1 <= phiNum[i][j][k]; n1++)
				{
					...
					for (n2 = 1; n2 <= phiNum[i][j][k]; n2++)
					{
						...
						if (ii == 1 && jj == 0)
						{
							dF = Tm + coni[i][j][k] * mv[i][j][k] - temp[i][j][k];
						}
						else if (ii == 0 && jj == 1)
						{
							dF = -(Tm + coni[i][j][k] * mv[i][j][k] - temp[i][j][k]);
		...
		for (j = start; j <= end; j++)
		{
			for (k = 0; k <= ndmz; k++)
			{
				...
				if (phi[0][i][j][k] > 0.0 && phi2[0][i][j][k] < 1.0)
				{
					...
					// if (istep > 200)
					// {
					// 	mv[i][j][k] = ml1 * (1.0 - kvi + kvi * log(kvi / kap1)) / (1.0 - kap1);
					// }
