...
# dfc = pd.read_csv(f"64-5dx-mo/con/2d{step}.csv", header=None)
# arrc = dfc[0].values
# matc = arrc.reshape(ny, nx)
# # plt.imshow(matc, origin='lower', cmap="binary", vmin=0.000, vmax=0.02)
# # plt.colorbar()
# plt.plot(x, matc[:, 0], marker="o")
# # plt.show()
...
# dfc = pd.read_csv(f"128-5dx-mo/con/2d{step}.csv", header=None)
# arrc = dfc[0].values
# matc = arrc.reshape(ny, nx)
# # plt.imshow(matc, origin='lower', cmap="binary", vmin=0.000, vmax=0.02)
# # plt.colorbar()
