...
int main()
{
	kap1 = 0.1; // partition coefficient
	...
	for (ix = 0; ix <= ndmx; ix++)
	{
		for (iy = 0; iy <= ndmy; iy++)
		{
			...
			{
				// if (ix < NDX / 8)
				// if ((ix - NDX / 2) * (ix - NDX / 2) + (iy - NDY / 2) * (iy - NDY / 2) < NDX / 16 * NDX / 16)
				if (ix + iy < NDX / 16)
				{
					phi[1][ix][iy][iz] = 1.0;
					conp[1][ix][iy][iz] = c1e;
...
// #pragma omp barrier
// 		// trap solute and calculate solid concentration
// 		for (i = start; i <= end; i++)
// 		{
// 			for (j = 0; j <= ndmy; j++)
// 			{
...
// 						km = ndmz;
// 					}
// 					if (phi[0][i][j][k] > 0.0 && phi2[0][i][j][k] < 0.5)
// 					{
