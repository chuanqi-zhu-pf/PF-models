...
int main()
{
	...
	ml = -605.3;
	kap = 0.105;
	Dl = 1.34e-7 * exp(-3.0e4 / RR / temp0) / dx / dx;
	Ds = 0.89e-4 * exp(-1.36e5 / RR / temp0) / dx / dx;
	dtime = 0.2 / Dl;
	...
	for (i = 0; i <= ndmx; i++)
	{
		for (j = 0; j <= ndmy; j++)
		{
			for (k = 0; k <= ndmz; k++)
			{
				// if ((i - NDX / 2) * (i - NDX / 2) + (j - NDY / 4) * (j - NDY / 4) + (k - NDZ / 2) * (k - NDZ / 2) < NDX / 32 * NDX / 32)
				if (i * i + j * j + k * k < NDX / 16 * NDX / 16)
				// if ((i - NDX * 3 / 4.5) * (i - NDX * 3 / 4.5) + (j - NDY * 3 / 4.5) * (j - NDY * 3 / 4.5) + (k - NDZ / 2) * (k - NDZ / 2) < NDX / 16 * NDX / 16)
				// // if (i < NDX / 16 && j < NDY / 2)
				{
					phi[1][i][j][k] = 1.0;
					phi[2][i][j][k] = 0.0;
				...
				// else if ((i) * (i) + (j - ndmy) * (j - ndmy) + (k - NDZ / 2) * (k - NDZ / 2) < NDX / 32 * NDX / 32)
			...
			for (k = 0; k <= ndmz; k++)
			{
				...
				for (n1 = 1; n1 <= phiNum[i][j][k]; n1++)
				{
					...
					for (n2 = 1; n2 <= phiNum[i][j][k]; n2++)
					{
						...
						if (ii > 0 && jj == 0)
						{
							dF = ((coni[i][j][k] - ce) * ml + Te - temp0) * S0;
						}
						else if (ii == 0 && jj > 0)
						{
							dF = -(((coni[i][j][k] - ce) * ml + Te - temp0)) * S0;
...
void datasave1d(int step)
{
	...
	char buffer[30];
	sprintf(buffer, "data/con/1d%d.csv", step);
	stream = fopen(buffer, "a"); // 書き込む先のファイルを追記方式でオープン

	for (i = 0; i <= ndmx; i++)
	{
		fprintf(stream, "%lf   ", con[i][0][0]);
		fprintf(stream, "\n");
	}
	...
	char buffercl[30];
	sprintf(buffercl, "data/phi/1d%d.csv", step);
	streamcl = fopen(buffercl, "a"); // 書き込む先のファイルを追記方式でオープン

	for (i = 0; i <= ndmx; i++)
	{
		fprintf(streamcl, "%lf   ", conp[0][i][0][0]);
		fprintf(streamcl, "\n");
	}
