				...
				for (k = 0; k <= ndmz; k++)
				{
					...
					for (n1 = 1; n1 <= phiNum[i][j][k]; n1++)
					{
						...
						for (n2 = 1; n2 <= phiNum[i][j][k]; n2++)
						{
							...
							if (ii == 1 && jj == 0)
							{
								dT = (1.0 - conp[0][i][j][k]/c10e)/(1.0-kap1) * dT0;
							}
							else if (ii == 0 && jj == 1)
							{
								dT = -(1.0 - conp[0][i][j][k]/c10e)/(1.0-kap1) * dT0;
		...
		for (i = start; i <= end; i++)
		{
			for (j = 0; j <= ndmy; j++)
			{
				for (k = 0; k <= ndmz; k++)
				{
					...
					fel = Dl * phi[0][i][j][k] * (conp0ip - conp[0][i][j][k]) / dx;
					fwl = Dl * phi[0][i][j][k] * (conp[0][i][j][k] - conp0im) / dx;

					fplx = Dl*(phi0ip-phi0im)*(conp0ip-conp0im)/4.0/dx/dx;
					...
					fnl = Dl * phi[0][i][j][k] * (conp0jp - conp[0][i][j][k]) / dx;
					fsl = Dl * phi[0][i][j][k] * (conp[0][i][j][k] - conp0jm) / dx;

					fply = Dl*(phi0jp-phi0jm)*(conp0jp-conp0jm)/4.0/dx/dx;
					...
					cddttl = fplx + fply +(fel - fwl + fnl - fsl + fol - fil) / dx;
